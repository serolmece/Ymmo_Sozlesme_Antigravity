...
// ADMIN REPORT: Reported Counts (Bildirilen Sözleşme Sayıları)
app.post('/api/admin/reports/reported-counts', async (req, res) => {
    const { year, contractType } = req.body;
    console.log('Reported Counts Request:', { year, contractType });

    try {
        const pool = await connectToDb();
        const request = pool.request();

        let whereClause = "WHERE S.Yil = @year";
        request.input('year', sql.Int, parseInt(year || new Date().getFullYear()));

        if (contractType && contractType !== 'HEPSİ') {
            whereClause += " AND S.SozlesmeTuru = @contractType";
            request.input('contractType', sql.NVarChar, contractType);
        }
        ...
        const query = `
            ...
            LEFT JOIN dbo.Uye U ON S.UyeId = U.Id
            ${whereClause}
            GROUP BY U.Ad, U.Soyad, S.SozlesmeTuru
            ORDER BY U.Ad ASC, U.Soyad ASC, S.SozlesmeTuru ASC
        `;
...
// ADMIN REPORT: Purchased Counts (Alınan Sözleşme Sayıları)
app.post('/api/admin/reports/purchased-counts', async (req, res) => {
    let { startDate, endDate, contractType } = req.body;
    console.log('Purchased Counts Request:', { startDate, endDate, contractType });
    ...
    try {
        ...
        if (startDate && endDate) {
            ...
            whereClause += " AND DO.createdAt >= @startDate";
            request.input('startDate', sql.NVarChar, `${startDate}T00:00:00.000`);
        } else if (endDate) {
            whereClause += " AND DO.createdAt <= @endDate";
            request.input('endDate', sql.NVarChar, `${endDate}T23:59:59.999`);
        }
            ...
            SELECT 
                ...
                (
                    ...
                    WHERE 
                        ...
                        ${startDate ? " AND SZ.CreatedAt >= @startDate" : ""}
                        ${endDate ? " AND SZ.CreatedAt <= @endDate" : ""}
                ) AS BildirenAdet
